...
export function InvoiceReceipt({ invoice, invoiceRef }: InvoiceReceiptProps) {
  ...
  return (
    <div 
      ref={invoiceRef}
      className={`w-full max-w-[800px] mx-auto bg-white p-6 sm:p-10 shadow-2xl relative overflow-hidden ${inter.className} print:p-0 print:shadow-none print:w-full print:max-w-none`}
      style={{ color: '#000' }}
    >
      {/* Top gold border */}
      ...
      <div className="flex flex-col sm:flex-row gap-6 sm:gap-8 items-center sm:items-start mt-4">
        {/* Logo Box */}
        <div className="bg-black rounded-lg p-4 sm:p-6 border border-[#cca354] shadow-sm flex flex-col items-center justify-center w-full max-w-[220px] sm:w-[220px] h-[220px] shrink-0">
        ...
        <div className="flex-1 pt-2 sm:pt-6 text-center sm:text-left">
          <h1 className={`text-3xl sm:text-5xl font-bold tracking-wide text-black uppercase ${playfair.className}`}>
            NEW DUKE & DUCHESS
          </h1>
          ...
          <div className="space-y-3 sm:space-y-4 mt-6 text-xs sm:text-sm text-neutral-700 inline-block text-left">
            <div className="flex items-start gap-3">
              <MapPin className="h-4 w-4 sm:h-5 sm:w-5 text-black shrink-0 mt-0.5" />
            ...
            <div className="flex items-center gap-3">
              <Phone className="h-4 w-4 sm:h-5 sm:w-5 text-black shrink-0" />
              <p>+91 89047 10353</p>
            </div>
      ...
      <div className="flex flex-col sm:flex-row gap-6 sm:gap-0">
        ...
        <div className="flex-1">
          ...
          <div className="grid grid-cols-[110px_1fr] sm:grid-cols-[130px_1fr] gap-y-2 text-xs sm:text-sm">
            ...
            {staffName && (
              <>
                <div className="text-neutral-500">Staff Name</div>
                <div className="font-medium">: {staffName}</div>
              </>
            )}
            <div className="text-neutral-500">Membership</div>
        ...
        <div className="block sm:hidden w-full h-[1px] bg-neutral-100" />
        ...
        <div className="flex-1">
          ...
          <div className="space-y-1">
            ...
            {invoice.customer?.membershipId && (
              <div className="flex items-center gap-2 text-xs sm:text-sm mt-1 text-neutral-600">
      ...
      <div className="w-full h-[1px] bg-neutral-200 my-6 sm:my-8" />
            ...
            <table className="w-full text-xs sm:text-sm min-w-[500px]">
              ...
              <tbody>
                {services.map((item: any, idx: number) => {
                  ...
                  return (
                    <tr key={idx} className="border-b border-[#cca354]/30 hover:bg-neutral-50/50">
                      <td className="py-3 sm:py-4 px-3 sm:px-4 text-neutral-800 break-words">{displayName}</td>
                      <td className="py-3 sm:py-4 px-2 sm:px-4 text-center text-neutral-600">{item.staffCode || item.staffName || "-"}</td>
                      <td className="py-3 sm:py-4 px-2 sm:px-4 text-center text-neutral-800">{item.quantity}</td>
                      <td className="py-3 sm:py-4 px-2 sm:px-4 text-right text-neutral-800">{formatCurrency(item.price)}</td>
                      <td className="py-3 sm:py-4 px-3 sm:px-4 text-right font-medium text-black">{formatCurrency(item.total)}</td>
                    </tr>
                  );
                })}
              </tbody>
            ...
            <table className="w-full text-xs sm:text-sm min-w-[500px]">
              ...
              <tbody>
                {products.map((item: any, idx: number) => (
                  <tr key={idx} className="border-b border-[#cca354]/30 hover:bg-neutral-50/50">
                    <td className="py-3 sm:py-4 px-3 sm:px-4 text-neutral-800 break-words">{item.name}</td>
                    <td className="py-3 sm:py-4 px-2 sm:px-4 text-center text-neutral-600">{item.staffCode || item.staffName || "-"}</td>
                    <td className="py-3 sm:py-4 px-2 sm:px-4 text-center text-neutral-800">{item.quantity}</td>
                    <td className="py-3 sm:py-4 px-2 sm:px-4 text-right text-neutral-800">{formatCurrency(item.price)}</td>
                    <td className="py-3 sm:py-4 px-3 sm:px-4 text-right font-medium text-black">{formatCurrency(item.total)}</td>
      ...
      <div className="flex flex-col sm:flex-row gap-6 mt-8 sm:mt-12">
        ...
        {Number(invoice.discount) > 0 || Number(invoice.membershipDiscount) > 0 || Number(invoice.couponDiscount) > 0 ? (
          <div className="flex-1 border border-[#cca354] rounded-lg p-5 sm:p-6 flex flex-col justify-center bg-[#cca354]/5 text-center sm:text-left">
            <div className="flex flex-col sm:flex-row items-center gap-3 sm:gap-4">
              <div className="bg-black text-[#cca354] p-2.5 sm:p-3 rounded-full shrink-0">
            ...
            <div className={`mt-3 sm:mt-4 text-base sm:text-lg text-neutral-600 ${greatVibes.className}`}>
        ...
        {/* Totals */}
        <div className="flex-1 w-full sm:w-auto border border-neutral-200 rounded-lg overflow-hidden flex flex-col justify-between self-end sm:self-auto min-w-[280px]">
          ...
          <div className="bg-black text-[#cca354] p-3 sm:p-4 flex justify-between items-center font-bold">
      ...
      <div className="mt-6 border border-[#cca354]/30 bg-[#cca354]/5 rounded-lg p-3 sm:p-4 flex flex-col sm:flex-row items-center gap-3 sm:gap-4 justify-center text-center sm:text-left">
        <div className="bg-[#cca354] text-white rounded-full p-1 sm:p-1.5 shrink-0">
          <Check className="h-4 w-4 sm:h-5 sm:w-5" />
        </div>
        <div>
          <div className="font-bold text-neutral-800 tracking-wide text-sm sm:text-base">PAYMENT SUCCESSFUL</div>
          <div className="text-xs sm:text-sm text-neutral-500 mt-0.5">Paid via {invoice.paymentMethod?.toUpperCase() || 'CASH'} | Thank you for your payment.</div>
        </div>
      </div>

      <div className="w-full h-[1px] bg-neutral-200 mt-8 sm:mt-10 mb-6" />
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-6 sm:gap-4 px-2 sm:px-4 pb-14 sm:pb-12">
        ...
        <div className="flex items-center gap-4 text-left sm:text-center sm:flex-col sm:gap-2">
          <div className="w-16 h-16 sm:w-20 sm:h-20 bg-neutral-100 rounded-sm flex flex-col items-center justify-center p-1 border border-neutral-300 shrink-0">
          ...
          <div className="space-y-1 sm:space-y-2">
            ...
            <div className="text-[10px] sm:text-xs text-neutral-600">Scan to leave us a<br className="hidden sm:block"/> Google Review</div>
        ...
        <div className="hidden sm:block w-[1px] h-16 bg-neutral-200" />
        <div className="block sm:hidden w-32 h-[1px] bg-neutral-200" />

        <div className="text-center space-y-3 sm:space-y-4">
          <div className="text-[#cca354] font-bold text-[10px] sm:text-xs uppercase tracking-widest">FOLLOW US</div>
          <div className="flex justify-center gap-3">
            <div className="bg-black text-[#cca354] h-6 w-6 sm:h-7 sm:w-7 flex items-center justify-center rounded-full text-[9px] sm:text-[10px] font-bold">IG</div>
            <div className="bg-black text-[#cca354] h-6 w-6 sm:h-7 sm:w-7 flex items-center justify-center rounded-full text-[9px] sm:text-[10px] font-bold">FB</div>
            <div className="bg-black text-[#cca354] h-6 w-6 sm:h-7 sm:w-7 flex items-center justify-center rounded-full text-[9px] sm:text-[10px] font-bold">WA</div>
          </div>
        </div>
        ...
        <div className="text-center sm:text-right">
          ...
          <div className="text-[10px] sm:text-xs text-neutral-600 leading-relaxed">
      ...
      {/* Bottom Black Bar */}
      <div className="absolute bottom-0 left-0 right-0 bg-black py-2 sm:py-2.5 text-center text-[#cca354] text-[8px] sm:text-[10px] tracking-[0.1em] sm:tracking-[0.2em] uppercase flex items-center justify-center gap-2 sm:gap-4">
        <span>♥</span>
